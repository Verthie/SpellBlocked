...
func _ready() -> void:
	...
	var y: int = 0
	while(Globals.liquid_height_level == 0):
		for x in range(0, 50, 10):
			var cell_id = get_cell_source_id(Vector2i(x, y))
			if cell_id == 4:
				Globals.liquid_height_level = map_to_local(Vector2i(x, y)).y - 4
				return
		y += 1

func _on_object_splash(splash_position: Vector2i):
