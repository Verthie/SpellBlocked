...
func _process(_delta) -> void:
	if tracking_position:
		var current_position = global_clip_length - timer.time_left
		...
		print("song position: ", true_position, "song time left: ", global_clip_length - true_position)
...
func set_checkpoint_clip_index(clip_index: int = 0):
...
func set_interactive_audioclip(clip_index: int = 0, track_position: bool = true, auto_override: bool = false) -> void:
	if audio_stream_interactive != null and clip_index != current_clip_index:
		if audio_stream_interactive.get_clip_name(clip_index) != null:
			...
			#print(audio_stream_interactive.get_transition_from_time(current_clip_index, clip_index))
			#print(audio_stream_interactive.get_transition_to_time(current_clip_index, clip_index))

			#if global_clip_length and current_clip_length != global_clip_length:
				#current_clip_length = global_clip_length
				# this is a filler clip
				#pass

			current_clip_index = clip_index

			var has_auto_mode: bool = audio_stream_interactive.get_clip_auto_advance(clip_index) if !auto_override else auto_override
...
func _on_game_restart() -> void:
	...
	if checkpoint_clip_index:
		...
		set_interactive_audioclip(audio_stream_interactive.initial_clip)
	await play_audio()
...
func _initiate_playback_position_tracking(current_clip_length: float = 0, has_auto_mode: bool = false) -> void:
	...
	timer = Timer.new()
	timer.timeout.connect(_add_to_clip_loop_counter)
	if has_auto_mode:
...
func _end_playback_position_tracking(kill_timer: bool = false) -> void:
	position_offset = 0
...
func _add_to_clip_loop_counter() -> void:
	clip_loop_amount += 1
	print("current amount of loops: ", clip_loop_amount)
